...
export default function FileRepository() {
    ...
    // feedbackLoadedMap: { [file.id]: true }  — set once the GET for that file resolves
    ...
    const fetchFeedbackForFiles = useCallback((files) => {
        ...
        // Fire all requests in parallel; each one updates its own card independently.
        files.forEach((f) => {
            feedbackApi
                .getFeedbackByFile(f.id)
                .then((list) => {
                    setFeedbackMap((prev) => ({ ...prev, [f.id]: Array.isArray(list) ? list : [] }));
                })
                .catch((err) => {
                    console.error(`Feedback fetch error for file ${f.id}:`, err);
                    setFeedbackMap((prev) => ({ ...prev, [f.id]: prev[f.id] ?? [] }));
                })
                .finally(() => {
                    setFeedbackLoadedMap((prev) => ({ ...prev, [f.id]: true }));
                });
        });
    }, []);
    ...
    const fetchMyFiles = useCallback(async () => {
        setMyLoading(true);
        setMyError(null);
        setFeedbackLoadedMap({});   // reset per-file flags on full reload
        try {
            const data = await fileSystemApi.getStudentFiles();
            const files = Array.isArray(data) ? data : [];
            setMyFiles(files);
            fetchFeedbackForFiles(files);   // fire-and-forget; each card updates itself
        } catch (err) {
    ...
    const renderFeedbackThread = (file) => {
        ...
        // Show section even if empty while refreshing, but not if never loaded
            ...
            <>
                ...
                <Box sx={{
                    ...
                    <Stack direction="row" alignItems="center" gap={1}>
                        ...
                        />
                        {isFileRefreshing && (
                            <CircularProgress size={10} sx={{ color: t.accentPrimary, ml: 0.5 }} />
                        )}
                    </Stack>
                </Box>
                ...
                <Stack
                    ...
                    sx={{ px: 0, pb: 2, bgcolor: `${t.accentPrimary}06` }}
                >
                    {feedbacks.map((fb) => (
                        <Box key={fb.feedbackId} sx={{ px: 2.5, pt: 2 }}>
                            ...
                            <Stack direction="row" gap={1.5} alignItems="flex-start">
                                ...
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <Button
                                        ...
                                        sx={{
                                            mt: 0.5, px: 0.8, fontSize: "0.71rem",
                                            color: replyOpen[fb.feedbackId] ? t.accentPrimary : t.textTertiary,
                                            minWidth: 0,
                                ...
                                <Box sx={{
                                    ...
                                    <Stack spacing={1.5}>
                                        {fb.replies.map((rep) => {
                                            ...
                                            const isMe = currentUserName !== "" && repSenderName !== "" &&
                                                repSenderName === currentUserName;
                                                    ...
                                                    <Box sx={{ flex: 1, minWidth: 0 }}>
                                                        ...
                                                        >
                                                            <Stack direction="row" gap={0.7} alignItems="center">
                                                                <Typography sx={{
                                                                    ...
                                                                    color: isMe ? t.accentPrimary : t.textPrimary,
                                                                }}>
                                                                    {rep.authorName || "Unknown"}
                                                                </Typography>
                                                                <Typography sx={{ fontSize: "0.63rem", color: t.textTertiary }}>
                                                        ...
                                                        {isEditing ? (
                                                            ...
                                                            <Paper elevation={0} sx={{
                                                                ...
                                                                bgcolor: isMe
                                                                    ? `${t.accentPrimary}10`
                                                                    : `${t.surfaceHover ?? theme.palette.action.hover}`,
                            ...
                            <Collapse in={Boolean(replyOpen[fb.feedbackId])}>
                                <Stack
                                    direction="row" gap={1.2} alignItems="flex-start"
                                    sx={{ mt: 1.5, ml: 5.5 }}
                                >
                                    <Avatar sx={{
                                        width: 26, height: 26, flexShrink: 0,
                                        bgcolor: t.accentPrimary, fontSize: "0.6rem", fontWeight: 700,
                                    }}>
    ...
    const renderMyFileCard = (file) => {
        ...
        return (
            <Paper key={file.id} elevation={0} sx={{
                ...
                {/* ── Feedback thread — rendered below card body ── */}
                {feedbackLoadedMap[file.id] && renderFeedbackThread(file)}

                {/* Spinner while this specific file's feedback is still loading */}
    ...
    return (
        <Box sx={{ width: "100%" }}>
            ...
            <TabPanel value={activeTab} index={0}>
                ...
                {/* Show spinner only during initial file load, NOT during feedback load */}
                {myLoading && (
