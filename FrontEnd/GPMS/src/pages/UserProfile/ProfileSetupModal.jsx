...
// ══════════════════════════════════════════════════════════════════════
// ثوابت خارج الـ component — عشان ما يصير re-create كل render
// ══════════════════════════════════════════════════════════════════════
...
// ── accent helper ────────────────────────────────────────────────────
// Student → برتقالي | Supervisor → أخضر | Admin → أحمر داكن
const getAccent = (role) => {
    if (role === "Supervisor") return "#5ba87d";
    if (role === "Admin") return "#c0504d";
    return "#d0895b"; // Student / default
};

// INPUT_SX كدالة لأن الـ accent بيتغير حسب الرول
...
// InnerCard خارج الـ component — يمنع unmount/remount للـ TextFields
function InnerCard({ icon: Icon, title, count, action, children, border, cardAlt, isDark, accent, a10, a22, labelSx }) {
    return (
        <Box sx={{ borderRadius: 2.5, border: `1px solid ${border}`, bgcolor: cardAlt, overflow: "hidden" }}>
            <Box sx={{
...
// Props:
//   open    — boolean
//   onDone  — callback بعد الحفظ
//   role    — "Student" | "Supervisor" | "Admin"  (من useAuth)
// ══════════════════════════════════════════════════════════════════════
export default function ProfileSetupModal({ open, onDone, role }) {
    const theme = useTheme();
    const isDark = theme.palette.mode === "dark";
    ...
    // ── Design tokens (accent ديناميكي حسب الرول) ──────────────────
    ...
    const chipSx = (sel) => ({
        ...
        bgcolor: sel ? accent : "transparent", color: sel ? "#fff" : textSec,
        border: `1px solid ${sel ? accent : border}`, cursor: "pointer",
        "&:hover": { bgcolor: sel ? `${accent}cc` : a10, borderColor: sel ? `${accent}cc` : a22 },
    });
    ...
    const handleNext = async () => {
        ...
        setError("");
        try {
            await UserProfileApi.createProfile(data);  // ✅ الإصلاح: UserProfileApi مش studentApi
            onDone(data);
        } catch (e) {
            setError(e?.response?.data?.message ?? "Failed to save profile, please try again.");
        } finally {
    ...
    const canNext =
        STEPS[step]?.id === "department" ? Boolean(department) :
            STEPS[step]?.id === "contact" ? Boolean(fullName.trim()) && Boolean(email.trim()) :
                true;

    const CurrentIcon = STEPS[step].icon;

    // ── Cover gradient حسب الرول ─────────────────────────────────
    const coverGradient = isDark
        ? role === "Supervisor"
            ? "linear-gradient(135deg, #182a20 0%, #101e15 100%)"
            : role === "Admin"
                ? "linear-gradient(135deg, #2a1818 0%, #1e1010 100%)"
                : "linear-gradient(135deg, #2a1f18 0%, #1e1510 100%)"
        : role === "Supervisor"
            ? "linear-gradient(135deg, #edf8f2 0%, #d4eddf 100%)"
            : role === "Admin"
