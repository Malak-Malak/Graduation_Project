...
// ── normalizeProfile ──────────────────────────────────────────────────────────
// Backend  →  UI
//   field (comma string "Frontend,Backend") → skills (array)
...
const normalizeProfile = (raw) => {
    ...
    // field من الباك هو string مفصول بـ comma → نحوّله لـ array
    ...
    return {
        fullName: raw.fullName ?? "",
        phoneNumber: raw.phoneNumber ?? "",
        department: raw.department ?? "",   // التخصص الأكاديمي
        skills: skillsFromField,           // الـ skills مستخرجة من field
        github: raw.gitHubLink ?? raw.github ?? "",
        linkedin: raw.linkedinLink ?? raw.linkedin ?? "",
        email: raw.personalEmail ?? raw.email ?? "",
        bio: raw.bio ?? "",
...
export default function ProfilePage() {
    ...
    const handleSave = async (updated) => {
        try {
            await studentApi.updateProfile(updated);
            // updated.skills هي array — نعيد normalize عشان يبقى الشكل موحّد
            ...
            // لو فشل الـ request خليّ الـ UI كما هو
        }
        setEditOpen(false);
    };
    ...
    return (
        <Box sx={{ maxWidth: 740, mx: "auto", pb: 4 }}>
            ...
            {/* ══ SKILLS (مستخرجة من field بالباك) ══════════════════════════ */}
            <Box mb={2}>
                <SectionCard
                    icon={CodeOutlinedIcon}
                    title="Skills"
