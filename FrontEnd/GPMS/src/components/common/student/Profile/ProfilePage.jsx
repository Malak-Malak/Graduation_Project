...
import GroupsOutlinedIcon from "@mui/icons-material/GroupsOutlined";
import PersonSearchOutlinedIcon from "@mui/icons-material/PersonSearchOutlined";
import WorkOutlineIcon from "@mui/icons-material/WorkOutline";   // للسوبر/أدمن بدل Groups
import EditProfileModal from "./EditProfileModal";
import ProfileSetupModal from "./ProfileSetupModal";
import { useAuth } from "../../../../contexts/AuthContext";
import studentApi from "../../../../api/handler/endpoints/studentApi";
...
export default function ProfilePage() {
    ...
    // ── role: يدعم user.role أو user.Role (بعض الـ backends ببعث Capital) ──
    ...
    useEffect(() => {
        studentApi.getProfile()
            .then((d) => {
                const normalized = normalizeProfile(d);
                setProfile(normalized);
                if (isProfileEmpty(normalized)) setSetupOpen(true);
            })
            .catch(() => {
                setProfile(null);
                setSetupOpen(true);
    ...
    // ── Design tokens (بعد loading check) ──────────────────────────
