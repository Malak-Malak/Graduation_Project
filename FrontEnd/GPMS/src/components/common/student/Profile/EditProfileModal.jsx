...
export default function EditProfileModal({ open, profile, onSave, onClose }) {
    ...
    const [activeSection, setActiveSection] = useState("department");
    const [department, setDepartment] = useState("");   // التخصص الأكاديمي
    const [skills, setSkills] = useState([]);   // تُرسَل كـ field للباك
    const [customSkillInput, setCustomSkillInput] = useState(""); // ← useState بدل useRef
    const [fullName, setFullName] = useState("");
    const [email, setEmail] = useState("");
    const [phoneNumber, setPhoneNumber] = useState("");
    const [linkedin, setLinkedin] = useState("");
    ...
    // ── إضافة skill مخصص — باستخدام useState (لا useRef) ─────────────────────
    const addCustomSkill = () => {
    ...
    const InnerCard = ({ children, sx = {} }) => (
        <Box sx={{
            ...
            bgcolor: cardAlt, overflow: "hidden", ...sx,
        }}>
            {children}
        </Box>
    );
            ...
            <Box sx={{ display: "flex", minHeight: 520 }}>
                ...
                <Box sx={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
                    ...
                    <DialogContent sx={{ flex: 1, px: 3, py: 3, overflowY: "auto" }}>
                        ...
                        {/* ══ SKILLS (تُرسَل كـ field للباك) ══════════ */}
                        {activeSection === "skills" && (
                            <Stack spacing={2}>
                                <Stack direction={{ xs: "column", md: "row" }} spacing={2}>

                                    {/* custom input — useState بدل useRef لحل مشكلة الـ focus */}
                                    <InnerCard sx={{ flex: 1 }}>
                                        <CardHeader icon={AddIcon} title="Add Custom Skill" />
                                        <Box sx={{ px: 2.5, py: 2 }}>
                                            <TextField
